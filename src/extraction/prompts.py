"""
Prompt templates for LLM-based battery specification extraction.

Provides zero-shot and few-shot prompting strategies for extracting
structured battery specifications from datasheet text.
"""
...
SCHEMA_DESCRIPTION = """
{
    ...
    "internal_resistance_mOhm": "float - Internal resistance in milliOhms (mΩ)",
...
4. Convert all values to the units specified in the schema (mAh, V, A, °C, g, Wh, mΩ).
5. For capacity: If stated in Ah, convert to mAh (multiply by 1000).
6. For current: If stated in mA, convert to A (divide by 1000).
7. For C-rate currents: Calculate actual current = C-rate × nominal capacity. For example, 0.2C for a 500mAh battery = 0.1A.
...
def build_zero_shot_prompt(datasheet_text: str) -> list:
    """
    Build a zero-shot prompt for battery specification extraction.
...
def build_few_shot_prompt(datasheet_text: str) -> list:
    """
    Build a few-shot prompt for battery specification extraction.
    Includes one example of (datasheet_text → JSON) to guide the LLM.
