...
\begin{tikzpicture}[
    ...
    block/.style={rectangle, draw, fill=blue!10, text width=3.8cm, text centered, rounded corners, minimum height=0.9cm, font=\small},
...
Le zero-shot atteint un F1 agr\'eg\'e de \textbf{96.4\%}, sup\'erieur au few-shot (\textbf{92.6\%}). Ce r\'esultat est contre-intuitif~: fournir un exemple au LLM n'am\'eliore pas syst\'ematiquement l'extraction, et peut m\^eme la d\'egrader.

Le few-shot introduit des erreurs sur la batterie 9059156, o\`u le mod\`ele hallucine plusieurs valeurs num\'eriques (capacit\'e 11\,500 au lieu de 10\,000\,mAh, courants de 2.3 au lieu de 2.0\,A). L'exemple fourni semble \textbf{biaiser} le mod\`ele vers des conversions de C-rate inappropri\'ees. 

Les deux strat\'egies inventent \'egalement des valeurs de temp\'erature de stockage et de capacit\'e minimale pour la LIR2450, absentes du datasheet.

En revanche, le few-shot excelle sur deux batteries~: le 16340 et le FFH3DS obtiennent tous deux un F1 de 100\%, l\`a o\`u le zero-shot commet une erreur de chimie (LiFePO4 class\'e comme Li-ion) sur le 16340.

En r\'esum\'e~:
\begin{itemize}[noitemsep]
    \item Le \textbf{zero-shot} est plus fiable en g\'en\'eral, avec seulement 4 FP et 1 FN.
    \item Le \textbf{few-shot} peut exceller sur certaines batteries (F1 parfait) mais introduit davantage d'hallucinations num\'eriques (9 FP).
    \item Les deux strat\'egies \'echouent \`a extraire le fabricant de la batterie 9059156.
\end{itemize}
...
\node[class, right=of bat] (spec) {Specification};
\node[class, below left=1.6cm and 0.2cm of bat] (mfr) {Manufacturer};
\node[class, below right=1.6cm and 0.2cm of bat] (chem) {Chemistry};
\node[class, right=of spec] (unit) {Unit};

\node[subclass, below left=1.3cm and -0.5cm of spec] (elec) {ElectricalSpec};
\node[subclass, below right=1.3cm and -0.5cm of spec] (therm) {ThermalSpec};
\node[subclass, below=0.5cm of elec] (phys) {PhysicalSpec};
\node[subclass, below=0.5cm of therm] (life) {LifecycleSpec};
...
\midrule
Membre 1 & Parsing PDF, extraction LLM, prompts zero-shot/few-shot, sch\'ema Pydantic \\
Membre 2 & Ontologie OWL, construction du graphe RDF, requ\^etes SPARQL \\
Membre 3 & Validation (r\`egles Python + SPARQL), \'evaluation, ground truth, rapport \\
\bottomrule
\end{tabular}
\end{table}
